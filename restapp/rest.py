...
def get_one_video():
    ...
    video_result = api.get_query(
        'videos', id=request.form['id_video'], part=request.form['part'])
...
def add_query(user_id, query_id):
    ...
    r = requests.post("http://worker:" + rest.config['WORKER_PORT'] +
                      "/" + user_id + "/add_query/" + query_id, json=request.get_json())
...
def add_video(user_id, query_id, query_type):
    ...
    rest.logger.debug("HEEEEEEEEEEEEEEEEEEEEE")
    rest.logger.debug(request.get_json())

    
    r = requests.post("http://worker:" + rest.config['WORKER_PORT'] + "/" + user_id + "/query/" + query_id + "/add_video/" + query_type, json=request.get_json())
...
def add_comments(user_id, query_id):
    ...
    results_query = requests.get(
        "http://restapp:" + rest.config['REST_PORT'] + "/" + user_id + "/queries/" + query_id + "/videos/")
    ...
    r = requests.post(
        "http://worker:" + rest.config['WORKER_PORT'] + "/" + user_id + "/add_comments/" + query_id, json=param)
...
def add_captions(user_id, query_id):
    ...
    param['list_vid'] = list_vid
    r = requests.post(
        "http://worker:" + rest.config['WORKER_PORT'] + "/" + user_id + "/add_captions/" + query_id, json=param)
...
def add_related(user_id, query_id):
    ...
    param['list_vid'] = list_vid
    r = requests.post(
        "http://worker:" + rest.config['WORKER_PORT'] + "/" + user_id + "/add_related/" + query_id, json=param)
...
if __name__ == '__main__':
    ...
    formatter = logging.Formatter(
        '%(filename)s ## [%(asctime)s] %(levelname)s == "%(message)s"', datefmt='%Y/%b/%d %H:%M:%S')
    handler = RotatingFileHandler(
        './' + rest.config['LOG_DIR'] + '/activity_rest.log', maxBytes=100000, backupCount=1)
    ...
    rest.run(debug=rest.config['debug_level'], host='0.0.0.0',
             port=rest.config['REST_PORT'], threaded=True)
